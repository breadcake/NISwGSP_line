...
public:
	...
	const vector<vector<vector<Point2> > >            & getLineSamplePoints() const;
	const vector<vector<vector<InterpolateVertex> > > & getLineMeshInterpolation() const;
	const vector<vector<vector<double> > >            & getLineTermU() const;
...
private:    
	...
	void writeImageOfMatchingPoints(const string & _name,
	...
	mutable vector<vector<vector<Point2> > >            line_sample_points;
	mutable vector<vector<vector<InterpolateVertex> > > line_mesh_interpolation;
	mutable vector<vector<vector<double> > >            line_term_u;
};
