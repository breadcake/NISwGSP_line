...
#ifndef NDEBUG
		writeImageOfMatchingPoints("1", match_pair,
			images_data[m1].mesh_2d->getVertices(), apap_matching_points[m1][m2], apap_overlap_mask[m1][m2]);
		//writeImageOfMatchingPoints("2", match_pair,
		//	apap_matching_points[m2][m1], images_data[m2].mesh_2d->getVertices(), apap_overlap_mask[m2][m1]);
#endif
    }
}
...
//  Added by breadcake on 2020/12/25.
void MultiImages::writeImageOfMatchingPoints(const string & _name,
											 const pair<int, int> & _match_pair,
											 const vector<Point2> & m1_fpts, const vector<Point2> & m2_fpts,
											 const vector<bool> & mask) const {
	vector<Point2> f1, f2;
	for (int i = 0; i < mask.size(); ++i) {
		if (mask[i]) {
			f1.emplace_back(m1_fpts[i]);
			f2.emplace_back(m2_fpts[i]);
		}
	}

	Mat image_of_feauture_pairs = getImageOfFeaturePairs(images_data[_match_pair.first].img,
														 images_data[_match_pair.second].img,
														 f1, f2, "matching_points");
	imwrite(parameter.debug_dir +
			"matching_points-" +
			images_data[_match_pair.first].file_name + "-" +
			images_data[_match_pair.second].file_name + "-" +
...
//  Added by breadcake on 2020/12/5.

bool draw_flag = false;
Point2f startPoint, endPoint;
vector<pair<Point2f, Point2f> > lines_pt;
void mouse_line(int event, int x, int y, int flags, void *utsc) {
	Mat &image = *(Mat*)utsc;
	...
	case CV_EVENT_LBUTTONDOWN:
		draw_flag = true;
		startPoint = Point2f(x, y);
		imshow("Lines", img2);
		break;
	case CV_EVENT_MOUSEMOVE:
		if (draw_flag) {
			line(img2, startPoint, Point2f(x, y), Scalar(0, 255, 255), 1, LINE_AA);
			imshow("Lines", img2);
		}
		break;
	case CV_EVENT_LBUTTONUP:
		draw_flag = false;
		endPoint = Point2f(x, y);
		lines_pt.emplace_back(make_pair(startPoint, endPoint));
		line(image, startPoint, endPoint, Scalar(0, 255, 255), 1, LINE_AA);
		imshow("Lines", image);
		break;
	default:
		break;
...
//  Added by breadcake on 2020/11/30.
...
const vector<vector<vector<Point2> > > & MultiImages::getLineSamplePoints() const
{
	if (line_sample_points.empty()) {
		line_sample_points.resize(images_data.size());
		for (int i = 0; i < images_data.size(); i++) {
			double lh = images_data[i].mesh_2d->lh;
			double lw = images_data[i].mesh_2d->lw;
			...
			vector<LineData> lines_data = images_data[i].getLines();

			vector<vector<pair<Point2f, Point2f>>> endpoints;
			endpoints.resize(images_data.size());
			for (int j = 0; j < lines_data.size(); j++)
			{
				endpoints[i].push_back(make_pair(lines_data[j].data[0], lines_data[j].data[1]));
			}
			...
			for (int j = 0; j < lines_count; ++j) {
				lines_width.emplace_back(lines_data[j].width);
				lines_length.emplace_back(lines_data[j].length);
				lines_slope.emplace_back((lines_data[j].data[1].y - lines_data[j].data[0].y) / (lines_data[j].data[1].x - lines_data[j].data[0].x));
			}
			double mean_length, std_length, mean_width, std_width;
			Statistics::getMeanAndSTD(lines_length, mean_length, std_length);
			Statistics::getMeanAndSTD(lines_width, mean_width, std_width);
			for (int j = 0; j < lines_count; j++) {
				//if ((lines_width[j] > 0.5*mean_width) && (lines_length[j] > 0.5*mean_length) && (lines_length[j] >= min_len))
				if (lines_length[j] >= min_len)
				{
			...
			if (LINE_INTERACTIVE) {
				Mat temp = images_data[i].img.clone();
				for (int j = 0; j < image_lines.size(); j++) {
					Scalar color(rand() % 256, rand() % 256, rand() % 256);
					line(temp, Point(image_lines[j][0], image_lines[j][1]),
						Point(image_lines[j][2], image_lines[j][3]), color, 2, LINE_AA);
				}
				//namedWindow("Lines", 0);
				namedWindow("Lines");
				imshow("Lines", temp);
				cout << "Please select lines in this picture, press any key to the next picture." << endl;
				setMouseCallback("Lines", mouse_line, (void *)&temp);
				if (draw_flag) {
					imshow("Lines", temp);
				}
				waitKey();
				for (int l = 0; l < lines_pt.size(); ++l) {
					if (norm(lines_pt[l].first - lines_pt[l].second) > min_len) {
						image_lines.emplace_back(lines_pt[l].first.x, lines_pt[l].first.y, lines_pt[l].second.x, lines_pt[l].second.y);
						image_lines_length.emplace_back(norm(lines_pt[l].first - lines_pt[l].second));
						image_lines_slope.emplace_back((lines_pt[l].second.y - lines_pt[l].first.y) / (lines_pt[l].second.x - lines_pt[l].first.x));
					}
				}
				cout << "第 " << i + 1 << " 张图片通过交互方式新增线条数: " << lines_pt.size() << endl;
				lines_pt.clear();
				destroyAllWindows();
			}

			Mat line_feature_img = images_data[i].img.clone();
			Mat sample_point_img = Mat::zeros(images_data[i].img.rows, images_data[i].img.cols, CV_8UC3);
			line_sample_points[i].resize(image_lines.size());
			for (int j = 0; j < line_sample_points[i].size(); j++) {
				int nl = cvCeil(image_lines_length[j] / diag);  //cvCeil to ensure each line has sample points > 2
				double ll = image_lines_length[j] / nl;
				float x = image_lines[j][0];
				float y = image_lines[j][1];
				int sample_points_num = 0;
				vector <Point2> sample_points;
				sample_points.reserve(nl + 1);
				float min_x = min(image_lines[j][0], image_lines[j][2]); float max_x = max(image_lines[j][0], image_lines[j][2]);
				float min_y = min(image_lines[j][1], image_lines[j][3]); float max_y = max(image_lines[j][1], image_lines[j][3]);
				while (sample_points_num < nl + 1) {
					sample_points.emplace_back(x, y);
					x += fabs(ll * cos(atan(image_lines_slope[j]))) * sgn(image_lines[j][2] - image_lines[j][0]);
					y += fabs(ll * sin(atan(image_lines_slope[j]))) * sgn(image_lines[j][3] - image_lines[j][1]);
					/*if (x > max_x || x < min_x || y > max_y || y < min_y)
						break;*/
					sample_points_num++;
				}
				//cout << "第 " << i + 1 << " 张图片第 " << j + 1 << " 条线的采样点数: " << sample_points_num << endl;
				Scalar color(rand() % 256, rand() % 256, rand() % 256);
				line(line_feature_img, Point(image_lines[j][0], image_lines[j][1]),
					Point(image_lines[j][2], image_lines[j][3]), color, 2, LINE_AA);
				line(sample_point_img, Point(image_lines[j][0], image_lines[j][1]),
					Point(image_lines[j][2], image_lines[j][3]), color, 1, LINE_AA);
				for (int k = 0; k < sample_points_num; ++k) {
					circle(sample_point_img, sample_points[k], 3, Scalar::all(255), 1, LINE_AA);
				}

				line_sample_points[i][j].reserve(sample_points_num);
				for (int ii = 0; ii < sample_points.size(); ii++)
					line_sample_points[i][j].emplace_back(sample_points[ii]);
			}
			imwrite(parameter.debug_dir + "line-result-" +
					images_data[i].file_name +
					+"-filterdiag" + images_data[i].file_extension, line_feature_img);
			imwrite(parameter.debug_dir + "line-result-" +
					images_data[i].file_name +
					+"-sample_point" + images_data[i].file_extension, sample_point_img);

		}
	}
	return line_sample_points;
}

const vector<vector<vector<InterpolateVertex> > > & MultiImages::getLineMeshInterpolation() const
{
	if (line_mesh_interpolation.empty()) {
		line_mesh_interpolation.resize(images_data.size());
		const vector<vector<vector<Point2> > > & line_sample_points = getLineSamplePoints();
		for (int i = 0; i < line_mesh_interpolation.size(); ++i) {
			line_mesh_interpolation[i].resize(line_sample_points[i].size());
			for (int j = 0; j < line_sample_points[i].size(); ++j) {
				line_mesh_interpolation[i][j].reserve(line_sample_points[i][j].size());
				for (int k = 0; k < line_sample_points[i][j].size(); ++k)
					line_mesh_interpolation[i][j].emplace_back(images_data[i].mesh_2d->getInterpolateVertex(line_sample_points[i][j][k]));
			}
		}
	}
	return line_mesh_interpolation;
}

const vector<vector<vector<double > > > & MultiImages::getLineTermU() const
{
	if (line_term_u.empty()) {
		line_term_u.resize(images_data.size());
		const vector<vector<vector<Point2> > > & line_sample_points = getLineSamplePoints();
		for (int i = 0; i < line_term_u.size(); ++i) {
			line_term_u[i].resize(line_sample_points[i].size());
			for (int j = 0; j < line_sample_points[i].size(); ++j) {
				line_term_u[i][j].reserve(line_sample_points[i][j].size());
				for (int k = 0; k < line_sample_points[i][j].size(); ++k)
					line_term_u[i][j].emplace_back(norm(line_sample_points[i][j][k] - line_sample_points[i][j].front()) /
						norm(line_sample_points[i][j].back() - line_sample_points[i][j].front()));
			}
		}
	}
	return line_term_u;
