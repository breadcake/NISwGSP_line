...
cv::Mat getImageOfFeaturePairs(const cv::Mat & img1,
	...
	int CIRCLE_RADIUS = 5, CIRCLE_THICKNESS = 1;
	if (_type == "matching_points") {
		CIRCLE_RADIUS = 3, CIRCLE_THICKNESS = -1;
	}
    const int LINE_THICKNESS   = 1;
    const int RGB_8U_RANGE     = 256;
    
    cv::Mat result = cv::Mat::zeros(max(img1.rows, img2.rows), img1.cols + img2.cols, CV_8UC3);
    ...
    for(int i = 0; i < f1.size(); ++i) {
        ...
        line(result, f1[i], f2[i] + Point2(img1.cols, 0), color, LINE_THICKNESS, cv::LINE_AA);
        circle(result, f2[i] + Point2(img1.cols, 0), CIRCLE_RADIUS, color, CIRCLE_THICKNESS, cv::LINE_AA);
    }
    return result;
}