...
void MeshOptimization::setWeightToLinePreservingTerm(const double _weight) {
	line_preserving_weight = _weight;
}
...
void MeshOptimization::reserveData_line(vector<Triplet<double> > & _triplets,
	...
	const bool local_similarity_term = local_similarity_weight;
	const bool global_similarity_term = (global_similarity_weight_beta || global_similarity_weight_gamma);
	const bool line_preserving_term = line_preserving_weight;

	int edge_count = (local_similarity_term || global_similarity_term) ? getEdgesCount() : 0;
	int similarity_equation_count = (edge_count) ? edge_count * DIMENSION_2D : 0;
	int edge_neighbor_vertices_count = (similarity_equation_count) ? getEdgeNeighborVerticesCount() : 0;

	int line_preserving_equation_count = (line_preserving_weight) ? getLinePreservingTermEquationCount() : 0;
	...
	line_preserving_equation.first = equation;
	line_preserving_equation.second = (line_preserving_term) ? line_preserving_equation_count : 0;

	_triplets.reserve(alignment_equation.second * 8 +
					  (local_similarity_term) * (edge_neighbor_vertices_count * 8 + edge_count * 4) +
					  (global_similarity_term) * (edge_neighbor_vertices_count * 8) +
					  (line_preserving_term) * (line_preserving_equation_count * 12) +
					  _start_index);
	_b_vector.reserve((global_similarity_term)* edge_neighbor_vertices_count * 4 +
					  _start_index);
}
...
void MeshOptimization::prepareLinePreservingTerm(vector<Triplet<double> > & _triplets,
	...
	if (line_preserving_equation.second) {
		...
		for (int i = 0; i < multi_images->images_data.size(); i++) {
			...
			for (int j = 0; j < line_interpolation[i].size(); j++) {
				for (int k = 1; k < line_interpolation[i][j].size() - 1; k++) {
					for (int dim = 0; dim < DIMENSION_2D; ++dim) {
...
int MeshOptimization::getLinePreservingTermEquationCount() const {
	int result = 0;
	const vector<vector<vector<InterpolateVertex> > > & line_interpolation = multi_images->getLineMeshInterpolation();
	for (int i = 0; i < line_interpolation.size(); i++)//pic
	{
		for (int j = 0; j < line_interpolation[i].size(); j++)//lines
		{
			result += line_interpolation[i][j].size() - 2;
		}
	}
	return result * DIMENSION_2D;
}
...
vector<vector<Point2> > MeshOptimization::getImageVerticesBySolving(vector<Triplet<double> > & _triplets,
	...
	const int equations = line_preserving_equation.first + line_preserving_equation.second;
