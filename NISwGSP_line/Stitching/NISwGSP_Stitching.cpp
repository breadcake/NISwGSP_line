...
void NISwGSP_Stitching::setWeightToLinePreservingTerm(const double _weight) {
	MeshOptimization::setWeightToLinePreservingTerm(_weight);
}
...
Mat NISwGSP_Stitching::solve_line(const BLENDING_METHODS & _blend_method) {
	const MultiImages & multi_images = getMultiImages();

	vector<Triplet<double> > triplets;
	vector<pair<int, double> > b_vector;

	reserveData_line(triplets, b_vector, DIMENSION_2D);
	...
	prepareAlignmentTerm(triplets);
	prepareSimilarityTerm(triplets, b_vector);
	prepareLinePreservingTerm(triplets, b_vector);
